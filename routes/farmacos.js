...
router.get('/', function(req, res, next) {
  ...
  .then(function (data) {
    let a= paginate.href(req)
    let countDocuments = data.data["queryCountFarmacos"]
    let totalPages = Math.ceil(countDocuments / limit)
//    res.json(data.data );
    res.render('farmacos/index', {paginate: a, farmacos: data.data["queryPageFarmacos"], title: 'Farmacos', totalPages: totalPages , currentPage: page })
  })
})
